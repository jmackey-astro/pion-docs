...
#. run serial code compilation script, and/or
#. run parallel code compilation script.
#. copy the executables to wherever you want to run the code.
...
to install. The script needs internet access to download the source code for the libraries.  
It also needs cmake, c, c++, fortran compilers to be installed on the system.
...
#. wind_test1.00000000.silo   initial conditions filename (set by user!).


**OPTIONAL ARGUMENTS**

#. op_criterion=1 	       output every n-time units (not n-steps)
#. opfreq_time=1.58e10	       output frequency (in code seconds)
#. cooling=0		       no cooling
#. redirect=test1	       redirect stdout to file ./test1info.txt
#. outfile=/path/to/results/wind_test1			       output file name with path (will be appended with step number)
#. cfl=0.1		       Courant number (<1 in 1D, <0.5 2D, <0.35 in 3D) and less with cooling etc.
...
==================   ===================================================================
"coordinates"        cylindrical (z,R) (2d only),
...
==================   ==================================================================
"solver"(important)  this is the solver to use for the flux calculation.
                     For the Euler equations (no magnetic fields) you can use
